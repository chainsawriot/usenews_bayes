...
## require(sjmisc)
## require(sjstats)
## import_formula <- brmsformula(res~offset(log(n))+(1|country)+log(import))

## get_prior(import_formula, data = media_summary, family = negbinomial())


## import_brms <- brm(res~offset(log(n))+(1|country)+log(import), data = media_summary, family = negbinomial(), control = list(adapt_delta = 0.9))
## summary(import_brms)

## require(lme4)

## import_glmer <- glmer.nb(res~offset(log(n))+(1|country)+log(import), data = media_summary)

## summary(import_glmer)

## import_mass <- MASS::glm.nb(res~offset(log(n))+country+log(import), data = media_summary)
